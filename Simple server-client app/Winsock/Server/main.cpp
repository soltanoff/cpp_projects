...
namespace ServerThreads
{
	...
	bool get_banlist(int &bytesSent, int clientNumber)
	{
		...
		for (int i = 0; i < banList.size(); i++)
		{
			...
			strcat(str, "\t pardon: ");
			strcat(str, itoa((t - banList[i].t), buf, 10));
			strcat(str, "\n");
		}
		strcat(str, "\0");
	...
	void accept_socket(SOCKET &AcceptSocket, sockaddr_in &ClientInfo, int &adr)
	{
		AcceptSocket = SOCKET_ERROR;
			while (AcceptSocket == SOCKET_ERROR)
				AcceptSocket = accept(m_socket, (sockaddr* ) &ClientInfo, &adr);
	}
...
public:
	...
	int init()
	{
		...
		if (m_socket == INVALID_SOCKET) // ¬ы¤вление ошибок
		{
